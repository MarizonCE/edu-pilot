...
def _get_data_and_validate(state: IngestGraphState) -> str:
    ...
    if not original_file_path_str:
        logger.error("The original_file_path_str is empty. 无法继续导入文件，提前终止导入！")
...
def _validate_basic_file(original_file_path_obj: Path) -> None:
    ...
    if not original_file_path_obj.exists():
        logger.error(f"{original_file_path_obj} 路径中不存在真实文件，"
    ...
    if not original_file_path_obj.is_file():
        logger.error(f"{original_file_path_obj} 路径存在，但是指向的不是文件，"
    ...
    if original_file_path_obj.is_symlink():
        logger.error(f"{original_file_path_obj} 是符号链接文件，"
                     f"无法继续导入文件，提前终止导入！请不要上传符号链接文件！")
    ...
    if original_file_size <= 0:
        logger.error(f"{original_file_path_obj} 内容为空！字节数：{original_file_size}。"
        ...
        logger.error(f"{original_file_path_obj} 文件过大！文件大小：{original_file_size / 1024 / 1024:.3f} MiB。"
                     f"无法继续导入文件，提前终止导入！请限制文件大小在 {MAX_FILE_SIZE_BYTES / 1024 / 1024:.0f} MiB 以内！")
...
def _get_suffix_and_validate(original_file_path_obj: Path) -> str:
    ...
    if original_file_suffix not in SUPPORTED_FILE_EXTENSIONS:
        logger.error(f"格式不支持！您上传的文件格式为 {original_file_suffix}。"
                     f"无法继续导入文件，提前终止导入！目前仅支持 {", ".join(SUPPORTED_FILE_EXTENSIONS)} 格式")
...
def _validate_mime(original_file_path_obj: Path, original_file_suffix: str, ) -> str:
    ...
    # 1. 获取上传的文件的 MIME 类型
    try:
        with open(original_file_path_obj, "rb") as f:
            # 读取 original_file_path_obj 指向的文件，通过文件内容或文件头判断文件真实的 MIME 类型；mime=True 表示读取 MIME 类型，而不是文件描述
            original_file_actual_mime = magic.from_buffer(f.read(2048), mime=True)

    except Exception as e:
        logger.exception(f"无法识别文件的真实类型！无法继续导入文件，提前终止导入！"
                         f"异常信息：{e}")
    ...
    if not supported_mime_types:
        logger.error(f"格式不支持！您上传的文件后缀为：{original_file_suffix}，未配置该格式的 MIME 白名单！"
                     f"无法继续导入文件，提前终止导入！请上传标准的文件！")
    ...
    if original_file_suffix == "pptx" and original_file_actual_mime == "application/octet-stream":
        try:
            ...
            logger.exception(f"上传的 {original_file_suffix} 文件不是有效的 ZIP/OOXML 容器！"
                             f"无法继续导入文件，提前停止导入！请确保您的文件是标准文件！")
            raise ValueError(f"上传的 {original_file_suffix} 文件不是有效的 ZIP/OOXML 容器！"
                             f"无法继续导入文件，提前停止导入！请确保您的文件是标准文件！") from e
        except OSError as e:
            logger.exception(f"上传的 {original_file_suffix} 文件读取失败！"
                             f"无法继续导入文件，提前停止导入！请确保您的文件是标准文件！")
    ...
    if original_file_actual_mime not in supported_mime_types:
        logger.error(f"格式不支持！您上传的文件后缀为：{original_file_suffix}，"
...
def _virus_scan(original_file_path_obj: Path) -> None:
    ...
    try:
        ...
        if not pyclamd_client.ping():
            logger.error("ClamAV 病毒扫描服务不可用！为确保安全，无法继续导入文件，提前终止导入！")
        ...
        logger.exception("ClamAV 病毒扫描服务不可用！为确保安全，无法继续导入文件，提前终止导入！")
        raise RuntimeError("ClamAV 病毒扫描服务不可用！"
...
def _validate_file_content(
    ...
    """通过解析器解析文件，判断是否实际格式是否符合，并更新对应的文件"""
    try:
        match original_file_suffix:
            ...
            case "pdf":
                ...
                state["pdf_path_str"] = original_file_path_str

            case "pptx":
                Presentation(original_file_path_str)
                state["pptx_path_str"] = original_file_path_str

            case "jpg" | "jpeg" | "png":
                with Image.open(original_file_path_obj) as image:
                    image.verify()
                state["image_path_str"] = original_file_path_str

            case "docx":
                Document(original_file_path_str)
                state["docx_path_str"] = original_file_path_str

            case "md":
                original_file_path_obj.read_text(encoding="utf-8")
                state["md_path_str"] = original_file_path_str

            case _:
                raise ValueError

    except Exception as e:
        logger.exception(f"{original_file_suffix} 文件内容解析失败，可能是损坏文件或伪装文件或格式不支持，"
                         f"文件名为 {original_file_path_obj.name}。无法继续导入文件，提前终止导入！")
...
def service_validate_file(state: IngestGraphState) -> IngestGraphState:
    ...
    # 6. 通过解析器校验文件格式
    _validate_file_content(original_file_path_obj, original_file_path_str, original_file_suffix, state)
